...
const InputGroup: React.FC<InputGroupProps> = ({
  ...
  return (
    <div className={className}>
      ...
      <div
        className={cn(
          "relative mt-2 [&_svg]:absolute [&_svg]:top-1/2 [&_svg]:-translate-y-1/2 [&_svg]:text-[#8A7A61] dark:[&_svg]:text-dark-6",
      ...
      >
        <input
          ...
          className={cn(
            "w-full outline-none transition disabled:cursor-default disabled:bg-gray-2 data-[active=true]:border-primary dark:disabled:bg-dark dark:data-[active=true]:border-primary",
            type === "file"
              ? getFileStyles(props.fileStyleVariant!)
              : "merchant-input text-dark placeholder:text-[#8A7A61] dark:text-white",
            props.iconPosition === "left" && "pl-12.5",
            props.height === "sm" && "h-10 py-2.5",
          )}
          required={required}
          disabled={disabled}
          data-active={active}
